...
public class Servidor implements Runnable {
    ...
    @Override
    public void run() {
        // Creamos el ServerSocket una sola vez
        try (ServerSocket servidor = new ServerSocket(puerto)) {
            while (!estado.equals("acertado")) {
                ...
                try (
                        // Creamos socket que contactara con el cliente
                    ...
                    if (numeroClienteString != null) {
                        ...
                        if (numeroCliente == numero) {
                            ...
                            System.out.println("SERVIDOR: El numero correcto es mas grande que " + numeroCliente);
                            estado = "pequeno";
                            escritor.println(estado);
                            // Por otro lado, si el numero del cliente es demasiado grande, mandamos estado grande
                        } else {
                            System.out.println("SERVIDOR: El numero correcto es mas pequeño que " + numeroCliente);
                            estado = "grande";
                            escritor.println(estado);
                        }
                    }
